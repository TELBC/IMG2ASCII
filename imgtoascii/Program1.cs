...
namespace imgtoascii
{
    class Program1
    {
        private static string density = "                ..'`^\",:;lIl!i><~+_-?][}{1)(|/tfjrxnuvczXYUJCLQ0OZmwqpdbkhao*#MW&8%B@$";
        private static int width_all=Console.LargestWindowWidth*3/4;
        private static int height_all=Console.LargestWindowHeight;
            ...
            using (var frame = new Mat())
            {
                ...
                while (true)
                {
                    ...
                    if (!h.IsInvalid)
                    {
                        ...
                        SmallRect rect = new SmallRect() { Left = 0, Top = 0, Right = (short)width_all, Bottom = (short)height_all };
                        for (int i = 0; i < width_all * height_all; i++)
                        {
                            ...
                            {//gray scaling with shadows
                                if (hexed[i] == '$' || hexed[i] == '@' || hexed[i] == 'B' || hexed[i] == '%' || hexed[i] == '8' || hexed[i] == '&' || hexed[i] == 'W')
                                    ...
                                    buf[i].Attributes = 0;
                                else if (hexed[i] == ':' || hexed[i] == ';' || hexed[i] == 'l' || hexed[i] == 'I' || hexed[i] == 'l' || hexed[i] == '!' || hexed[i] == 'i')
                                    buf[i].Attributes = 8;
                                else
                                    buf[i].Attributes = 7;
                            }
